...
<?php $sucursales = SucursalData::verid($_GET['tid']); ?>
<div class="main-content">
    <div class="page-content">
        <div class="container-fluid">
            ...
            <div class="profile-foreground position-relative mx-n4 mt-n4">
                <div class="profile-wid-bg">
                    <img src="storage/per/logo.png" alt="" class="profile-wid-img" />
                </div>
            </div>
            <div class="pt-4 mb-4 mb-lg-3 pb-lg-4">
                <div class="row g-4">
                    <div class="col-auto">
                        <div class="avatar-lg">
                            <img src="storage/per/logo.png" alt="user-img" class="img-thumbnail rounded-circle" />
                        </div>
                    </div>
                    <!--end col-->
                    <div class="col">
                        <div class="p-2">
                            <h3 class="text-white mb-1"><?= $sucursales->nombre; ?></h3>
                            <p class="text-white-75"><i class="ri-phone-fill me-1 text-white-75 fs-16 align-middle"></i> <?= $sucursales->telefono; ?></p>
                            <div class="hstack text-white-50 gap-1">
                                <div class="me-2"><i class="ri-map-pin-user-line me-1 text-white-75 fs-16 align-middle"></i><?= $sucursales->direccion; ?></div>
                                <div>
                                    <i class="ri-building-line me-1 text-white-75 fs-16 align-middle"></i>IMPORT AND EXPORT
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-lg-auto order-last order-lg-0">
                        <div class="row text text-white-50 text-center">
                            <div class="col-lg-12 col-12">
                                <div class="p-2">
                                    <h4 class="text-white mb-1"><?php echo count(SucursalData::vercontenidos()) ?></h4>
                                    <p class="fs-14 mb-0">Total de Registros</p>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <ul class="nav nav-pills animation-nav profile-nav gap-2 gap-lg-3 flex-grow-1" role="tablist">
                            <li class="nav-item">
                                <a class="nav-link fs-14" data-bs-toggle="tab" href="#overview-tab" role="tab">
                                    <i class="ri-airplay-fill d-inline-block d-md-none"></i> <span class="d-none d-md-inline-block">Información</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link fs-14 active" data-bs-toggle="tab" href="#documents" role="tab">
                                    <i class="ri-folder-4-line d-inline-block d-md-none"></i> <span class="d-none d-md-inline-block">Almacén</span>
                                </a>
                            </li>
                        </ul>
                    </div>
                    <div class="tab-content pt-4 text-muted">
                        ...
                        <div class="tab-pane active" id="documents" role="tabpanel">
                            <div class="card">
                                <div class="card-body">
                                    <div class="d-flex align-items-center mb-4">
                                        <h5 class="card-title flex-grow-1 mb-0">Lista de registro de Almacenes</h5>
                                        <div class="flex-shrink-0">
            ...
            <form class="tablelist-form" autocomplete="off" method="post" action="index.php?action=registro">
                        ...
                        <div class="live-preview">
                            <div class="row gy-0">
                                <div class="col-xxl-12 col-md-12">
                                    <label for="responsable">Sucursal</label>
                                    <select class="form-control" id="responsable_" name="responsable">
                                    </select>
                                </div>
                                <div class="col-xxl-12 col-md-12">
                                    <label for="customername-field" class="form-label">Almacén</label>
                ...
                <div class="modal-footer">
                    <div class="hstack gap-2 justify-content-end">
                        ...
                        <input type="hidden" name="sucursal" class="form-control" required value="<?= $sucursales->id; ?>" />
...
<script>
    $(document).ready(function() {
        $('#customerTable').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": "index.php?action=apialmacen",
            "columns": [
                { "data": "nombre" },
                { "data": "responsables" },
                { "data": "descripcion" },
                { "data": "estado",
                    "render": function(data, type, row) {
                        if(data == 1) {
                            return '<i class="ri-check-fill text-success"></i>';
                        } else {
                            return '<i class="ri-close-fill text-danger"></i>';
                        }
                    }
                },
                { 
        ...
        $('#showModal').on('show.bs.modal', function() {
            if ($('#responsable option').length <= 1) {
                $.ajax({
        ...
        $('#customerTable').on('click', '.edit-btn', function() {
            let id = $(this).data('id');
            $.get("index.php?action=getalmacen&id=" + id, function(data) {
                ...
                $.ajax({
                    ...
                    success: function(responsables) {
                        $('#responsable_').empty();
                        $('#responsable_').append($('<option>', {value: '', text: 'Elegir'}));
                        responsables.forEach(function(responsable) {
                            $('#responsable_').append($('<option>', {
                                value: responsable.id,
                                text: responsable.nombre + ', ' + responsable.apellido
                            }));
                        });
                        $('#responsable_').val(data.responsable);
                    }
                });
                $('#ids').val(data.id);
