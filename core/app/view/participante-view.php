                    ...
                    <div class="card shadow">
                        <div class="card-header bg-primary text-white d-flex align-items-center justify-content-between" style="padding: 0.5rem 1rem;">
                            ...
                            <div class="ms-auto">
                                ...
                                <button type="button" class="btn btn-light me-2 text-primary btn-sm" data-bs-toggle="modal" data-bs-target="#impotarparticipante">
                                    <i class="ri-add-line align-bottom me-1"></i> Importar
                                </button>
                                <button type="button" class="btn btn-light text-info btn-sm" data-bs-toggle="modal" data-bs-target="#exampleModalLong">
    ...
    <div class="modal-content border-5">
      <div class="modal-header bg-info">
        ...
        <form enctype="multipart/form-data" method="POST" class="tablelist-form" autocomplete="off" action="index.php?action=registro">
            <div class="modal-body bg-light">
                <div class="mb-4">
                    ...
                    <div class="d-flex align-items-center">
                        <a href="storage/per/plantilla.xlsx">
          ...
          <div class="modal-footer bg-info d-flex justify-content-between pb-0">
              <input class="form-control" type="hidden" name="actions" value="25">
              <input class="form-control" type="hidden" name="tid" value="<?= $_GET['tid'];?>">
              <button type="button" class="btn btn-outline-light me-2" data-bs-dismiss="modal">Cerrar</button>
...
<div class="modal fade" id="basededatos" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        ...
        <form enctype="multipart/form-data" method="POST" class="tablelist-form" autocomplete="off" action="index.php?action=registro">
            ...
            <div class="modal-footer bg-info d-flex justify-content-between pb-0">
              <input class="form-control" type="hidden" name="actions" value="26">
              <input class="form-control" type="hidden" name="tid" value="<?= $_GET['tid'];?>">
              <button type="button" class="btn btn-outline-light me-2" data-bs-dismiss="modal">Cerrar</button>
              <button type="submit" class="btn btn-light">Guardar registro</button>
            </div>
        ...
        $('#customerTable').DataTable({
            ...
            "columns": [
                ...
                { 
                    "data": null,
                    "render": function (data, type, row) {
                        return '<button class="btn btn-primary btn-sm edit-btn" data-id="' + row.id + '"><i class="ri-edit-2-fill"></i></button>' + ' ' 
                        + '<button class="btn btn-danger btn-sm delete-btn" data-id="' + row.id1 + '"><i class="ri-delete-bin-2-line"></i></button>';
                    }
                }
            ],
            "language": {
        ...
        $('#AtualizarOrden').on('shown.bs.modal', function () {
            if (!tableLoaded) {
                var table = $('#dynamicTable').DataTable({
                    ...
                    // Crear Sortable en todo el tbody
                    new Sortable(rows.closest('tbody')[0], {
                        ...
                        onMove: function(evt) {
                            if (ligaOrden === 'libre') {
                                ...
                                // Restringir movimiento si el Elo es diferente cuando el orden no es libre
                                var draggedElo = $(evt.dragged).attr('data-elo');
                                var targetElo = $(evt.related).attr('data-elo');
                                
                                // Permitir mover solo si los Elos son iguales
