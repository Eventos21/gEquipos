...
<script>
    $(document).ready(function() {
        $('#customerTable').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": "index.php?action=apimequipos&club=" + <?= $u->club; ?>,
            "columns": [
                { "data": function(row) {
                        return row.nombre ;
                    }
                ...
                { "data": function(row) {
                        return row.capitanes + ' ' + "<span style='color: blue;'>(" + moment(row.nacimiento2).format('DD-MM-YYYY') + ")</span>";
                    }
                },
                { "data": function(row) {
                ...
                { 
                    "data": null,
                    "render": function (data, type, row) { 
                        return '<button disabled title="Enviar a la Federación para su validación" class="btn btn-warning btn-sm federacion-btn" data-id="' + row.id + '"><i class="ri-telegram-fill"></i></button>' + ' ' + '<a href="participante1?tid=' + row.id + '&td=2" class="btn btn-info btn-sm"><i class="ri-group-fill"></i></a>' + ' ' + '<button disabled title="Actualizar registro" class="btn btn-primary btn-sm edit-btn" data-id="' + row.id + '"><i class="ri-edit-2-fill"></i></button>' + ' ' + '<button disabled title="Eliminar registro" class="btn btn-danger btn-sm delete-btn" data-id="' + row.id + '"><i class="ri-delete-bin-2-line"></i></button>';
                    }
                }
            ],
            "language": {
