                ...
                { "data": null,
                    "render": function(data, type, row) {
                        ...
                        if (row.nombregrupo.toLowerCase() === 'división de honor') {
                            buttons += ' <button title="Jornada" class="btn btn-primary btn-sm registro1-btn" data-id="' + row.id + '"><i class="ri-checkbox-multiple-fill"></i></button>';
                        } else {
                            buttons += ' <button title="Jornada" class="btn btn-primary btn-sm registro-btn" data-id="' + row.id + '"><i class="ri-checkbox-multiple-fill"></i></button>';
                        }

                        // buttons += ' <button title="Jornada" class="btn btn-warning btn-sm jornada-btn" data-id="' + row.id + '"><i class="ri-checkbox-multiple-fill"></i></button>';
            ...
            $.get("index.php?action=apivalorequipo&sala=" + id, function(data) {
                ...
                if (tipojuego[Object.keys(tipojuego)[0]].toLowerCase() === 'división de honor') {
                    ...
                    for (let i = 0; i < encuentros.length; i++) {
                        let row = `<tr><td>Ronda ${i + 1}</td>`;
                        row += `<td><input type="datetime-local" name="fecha_encuentro[]"></td>`;
                ...
                "columns": [
                    ...
                    { 
                        ...
                        "render": function(data) {
                            return '<input style="font-size: 9px" type="datetime-local" class="form-control" name="fecha_encuentro_[]" value="' + data + '">';
                        }
                    },
                    { "data": null,
                        "render": function(data, type, row) {
      ...
      <form enctype="multipart/form-data" class="tablelist-form" autocomplete="off" method="post" action="index.php?action=registro">
        <div class="modal-body bg-light">
          ...
          <div class="modal-footer bg-info d-flex justify-content-between pb-0">
              ...
              <button type="button" class="btn btn-outline-light me-2" data-bs-dismiss="modal">Cerrar</button>
              <button type="submit" class="btn btn-light" id="actualizar-btn">Actualizar</button>
            </div>
        </form>
    </div>
  </div>
      ...
      <form enctype="multipart/form-data" method="POST" class="tablelist-form" autocomplete="off" action="index.php?action=registro">
        <div class="modal-body bg-light">
            ...
            <div class="row">
                ...
                <div id="importar" style="display: none;" class="col-md-4">
                ...
                <script>
                    $(document).ready(function() {
                        ...
                        $('#liga-field').on('change', function() {
                            ...
                            // Verificar si el código es "LFARE"
                            if (selectedCodigo == "LFARE") {
