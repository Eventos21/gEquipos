...
<!-- <script>
    ...
    var table = $('#customerTable').DataTable({
        ...
        "columns": [
            { 
                ...
                "render": function (data, type, row, meta) {
                    ...
                    if (prevData && prevData.is_separator) {
                        table.row(meta.row).index(1); // Reiniciar el contador a 1 después del separador
                        return 1;
                    } else {
                        var count = 1;
            ...
            { "data": "empates" },
            { "data": "derrotas" },
            { "data": "incomparecencias" },
            { "data": "puntos" },
            { "data": "olimpico" },
            { "data": "sonnenborg_berger" },
            {
                "data": null,
                "render": function(data, type, row) {
                    if (row.is_separator) return ''; // No mostrar campo de sanción para el separador
                    return '<input style="font-size: 9px; width: 50px;" type="number" name="ubicacioninvitario_[]">';
                }
            },
        ],
...
<!-- <script>
    function updateSancion(element, sala_id, equipo) {
    var sancion = element.value;

    $.ajax({
        url: 'index.php?action=updatesancion', // Ruta del script que maneja la actualización en el servidor
        type: 'POST',
        data: {
        ...
        success: function(response) {
            ...
            // Aquí puedes manejar la respuesta del servidor si necesitas
        },
...
$(document).ready(function() {
    var table = $('#customerTable').DataTable({
        ...
        "columns": [
            ...
            { 
                "data": "equipo",
                "defaultContent": "" // Asegurarse de que haya contenido predeterminado si el campo está vacío
            },
            ...
            { "data": "sonnenborg_berger", "defaultContent": "" },
            {
                ...
                "render": function(data, type, row) {
                    if (row.is_separator) return ''; // No mostrar campo de sanción para el separador
                    return '<input style="font-size: 9px; width: 50px;" type="number" name="ubicacioninvitario_[]" onchange="updateSancion(this, \'' + row.sala_id + '\', \'' + row.equipo + '\')" value="' + (data.sancion || '') + '">';
                }
            },
        ],
        "rowCallback": function(row, data, index) {
